...
export default function ReviewModal({ onClose, isOpen = true }: Props) {
  useModalTheme(isOpen);
  const t = useT();
  ...
  useHardwareBack(() => setStep(1), isOpen && step >= 2); // Pops Step 2 -> 1
  useHardwareBack(() => setStep(2), isOpen && step >= 3); // Pops Step 3 -> 2

  // Reset all state when modal closes — replaces unmount-based reset
  useEffect(() => {
    if (!isOpen) {
      setStep(1);
